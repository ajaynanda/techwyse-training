...
const findteamByid = ((req, res) => {
    return new Promise((resolve, reject) => {
        const id = req.params.id
        const pindex = req.params.p
        const cindex = req.params.c
        treedb.findById(id).then(async (result) => {
            const arrays = result.Childrens
          const array=  [result.Childrens[pindex], result.Childrens[cindex]] = [result.Childrens[cindex], result.Childrens[pindex]]
            await treedb.findByIdAndUpdate({ _id: req.params.id }, { $unset: { 'Childrens': [] } })
            await treedb.findByIdAndUpdate({ '_id': req.params.id }, { $push: { Childrens: arrays } }).then((sol) => {
                resolve({ Success: true, message: "team data", data: sol })
            }).catch(err => {
                console.log(err);
                reject(err)
            })
    ...
    return new Promise((resolve, reject) => {
        ...
        treedb.findOneAndUpdate({ _id: id }, { $pull: { 'Childrens': { _id: req.params.ids } } }).then(async (result) => {
            ...
            if (results) {
                ...
                users.Childrens.splice(ia, 0, datas)
                await treedb.updateOne({ _id: req.params.addid }, { $unset: { Childrens: [] } })
                await treedb.findOneAndUpdate({ '_id': req.params.addid }, { $push: { 'Childrens': users.Childrens } }).then((data) => {
...
module.exports = { add, addchild, findteam, deletechild, deleteteam, updatechild, updateteam, arrayremoveitem, findteamByid }